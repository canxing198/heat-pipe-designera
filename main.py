...
with tab1:
    # 初始化 session_state，确保 design_result 始终存在
    if 'design_result' not in st.session_state:
        st.session_state.design_result = None

    # 在“开始设计”按钮点击时，执行设计逻辑并保存结果到 session_state
    if st.button("🚀 开始设计"):
    ...
    # 在“导出 Excel”按钮点击时，从 session_state 中读取结果并导出
    if st.button("📤 导出 Excel 工艺卡"):
        if st.session_state.design_result:  # 如果设计结果存在
            export_excel(st.session_state.design_result)
        else:
            st.warning("⚠️ 请先点击‘开始设计’以生成结果！")
